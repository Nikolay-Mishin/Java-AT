...
export const getReportInfo = async (type, copy = true, i = {}) => {
    ...
    if (setReport) {
        ...
        if (copy) {
            i = fileName(setFFmpeg ? input : o, true);
            ...
            const reportCopy = setFFmpeg ? file : file.replace(reportName, `${i}-${reportName}`);
            //log(file);
            log(reportCopy);
...
export const
    ...
    setColorspace = (i, format, fast = 0) => {
        ...
        const { range: irange, primaries: iprimaries, trc: itrc, space: ispace, chroma = default_chroma } = ffColor;
        ...
        const setCspace = (p, con = c_space) => setParam(con, getSpaceP(space, p));
        ...
        const _scale_range = !scale_range ? '' : `:out_range=${range}${setCspace('out_color_matrix', scale_space)}${setChroma('out_chroma_loc', chroma_loc)}`;
        ...
        const c_params = !setSpace ? '' : `range=${range}:colorprim=${primaries}:transfer=${trc}${setCspace('colormatrix', p_space)}:chromaloc=${chromaLocType - 1}`;
